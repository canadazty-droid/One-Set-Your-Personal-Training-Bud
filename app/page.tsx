...
function OneTapHome({language,history,equipment,setEquipment,listening,voiceTranscript,listen,request,setRequest,submitRequest,t,duration,setDuration,focus,noviceMode,setNoviceMode,building,advancedReady,build,openBuilder,openAdvanced}:{language:Language;history:WorkoutLog[];equipment:Equipment;setEquipment:(value:Equipment)=>void;listening:boolean;voiceTranscript:string;listen:()=>void;request:string;setRequest:(value:string)=>void;submitRequest:()=>void;t:Copy;duration:number;setDuration:(value:number)=>void;focus:Focus;noviceMode:boolean;setNoviceMode:(value:boolean)=>void;building:boolean;advancedReady:boolean;build:(focus:Focus,area?:BuildArea)=>void;openBuilder:()=>void;openAdvanced:()=>void}){
  ...
  const [specificAreasOpen,setSpecificAreasOpen]=useState(false);
  useEffect(()=>{setSelectedFocus(focus);setArea("all");setSpecificAreasOpen(false)},[focus]);
  ...
  const areas:Record<Focus,Array<{id:BuildArea|"all";zh:string;en:string}>>={
    full:[{id:"all",zh:"全身",en:"Full body"},{id:"chest",zh:"胸部",en:"Chest"},{id:"back",zh:"背部",en:"Back"},{id:"lower",zh:"臀腿",en:"Glutes + legs"},{id:"core",zh:"核心",en:"Core"}],
    ...
    lower:[{id:"all",zh:"下肢整体",en:"All lower"},{id:"glutes",zh:"臀部",en:"Glutes"},{id:"quads",zh:"股四头",en:"Quads"},{id:"hamstrings",zh:"后侧链",en:"Hamstrings"}],
  ...
  return <section className="one-tap-home simple-home" aria-label="One tap workout builder">
    ...
    {customOpen&&<section id="home-customize" className="home-customize">
      ...
      <div className="one-tap-focus">{focusChoices.map(item=><button key={item} className={selectedFocus===item?"active":""} onClick={()=>{setSelectedFocus(item);setArea("all");setSpecificAreasOpen(false)}} aria-pressed={selectedFocus===item}><i><AppIcon name={labels[item].icon}/></i><span>{zh?labels[item].zh:labels[item].en}</span></button>)}</div>
      <button className={`one-tap-specific-toggle ${specificAreasOpen?"active":""}`} onClick={()=>setSpecificAreasOpen(open=>!open)} aria-expanded={specificAreasOpen}><span><small>{zh?"细分训练部位":"SPECIFIC TARGET"}</small><strong>{area==="all"?(zh?"胸、背、肩、手臂、臀腿、核心…":"Chest, back, shoulders, arms, glutes, core…"):(zh?`已选择：${selectedArea.zh}`:`Selected: ${selectedArea.en}`)}</strong></span><b><AppIcon name={specificAreasOpen?"minus":"plus"}/></b></button>
      {specificAreasOpen&&<div className="one-tap-area"><section>{areas[selectedFocus].map(choice=><button key={choice.id} className={area===choice.id?"active":""} aria-pressed={area===choice.id} onClick={()=>setArea(choice.id)}>{zh?choice.zh:choice.en}</button>)}</section></div>}
      <div className="home-control-heading"><small>02</small><strong>{zh?"练多久？":"How long?"}</strong></div>
      <div className="home-duration">{[20,30,45,60].map(value=><button key={value} className={duration===value?"active":""} aria-pressed={duration===value} onClick={()=>setDuration(value)}><strong>{value}</strong><small>{zh?"分钟":"MIN"}</small></button>)}</div>
      <button className={`one-tap-starter ${noviceMode?"active":""}`} onClick={()=>setNoviceMode(!noviceMode)} aria-pressed={noviceMode}><i><AppIcon name={noviceMode?"check":"starter"}/></i><span><small>{zh?"新手模式":"STARTER MODE"}</small><strong>{noviceMode?(zh?"已开启：优先简单动作和充分休息":"On: easier movements and more recovery"):(zh?"第一次训练？使用更简单动作和充分休息":"New to training? Easier moves & more recovery")}</strong></span><b>{noviceMode?(zh?"已开启":"ON"):(zh?"开启":"TURN ON")}</b></button>
      <button className="one-tap-primary" disabled={building} aria-busy={building} onClick={runBuild}><span>{building?(zh?"正在生成…":"Building…"):(zh?"按我的选择生成":"Build with my choices")}<small>{duration} MIN · {zh?selectedArea.zh:selectedArea.en}</small></span><b><AppIcon name="arrow-right"/></b></button>
